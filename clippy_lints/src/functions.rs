use crate::utils::{
    function_header_span, iter_input_pats, match_def_path, qpath_res, snippet, snippet_opt, span_help_and_lint,
    span_lint, span_lint_and_then, type_is_unsafe_function,
};
use matches::matches;
...
impl<'a, 'tcx> LateLintPass<'a, 'tcx> for Functions {
    ...
    fn check_item(&mut self, cx: &LateContext<'a, 'tcx>, item: &'tcx hir::Item) {
        ...
        if let hir::ItemKind::Fn(ref decl, ref _header, ref _generics, ref body_id) = item.kind {
            if let Some(attr) = attr {
                check_needless_must_use(cx, decl, item.hir_id, item.span, attr);
                return;
            }
    ...
    fn check_impl_item(&mut self, cx: &LateContext<'a, 'tcx>, item: &'tcx hir::ImplItem) {
        if let hir::ImplItemKind::Method(ref sig, ref body_id) = item.kind {
            let attr = must_use_attr(&item.attrs);
            if let Some(attr) = attr {
                check_needless_must_use(cx, &sig.decl, item.hir_id, item.span, attr);
...
fn check_needless_must_use(
    ...
    } else if !attr.is_value_str() && returns_must_use(cx, item_id) {
        span_help_and_lint(
            cx,
            DOUBLE_MUST_USE,
            function_header_span(cx, item_span),
...
fn returns_must_use(cx: &LateContext<'_, '_>, id: hir::HirId) -> bool {
    is_must_use_ty(
        cx,
        cx.tcx.typeck_tables_of(id.owner_def_id()).liberated_fn_sigs()[id].output(),
    )
}
...
impl<'a, 'tcx> intravisit::Visitor<'tcx> for StaticMutVisitor<'a, 'tcx> {
    fn visit_expr(&mut self, expr: &'tcx hir::Expr) {
        ...
        match expr.kind {
            ...
            Assign(ref target, _) | AssignOp(_, ref target, _) |
            AddrOf(hir::Mutability::MutMutable, ref target) => {
